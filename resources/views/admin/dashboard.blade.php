...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
    ...
    <a href="{{ route('admin.pesanan.index') }}" class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:border-orange-400 transition">
        <p class="text-sm text-gray-500">Total Pesanan</p>
        <p class="text-3xl font-bold text-gray-900 mt-2">{{ $totalPesanan }}</p>
    </a>
    <div class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <p class="text-sm text-gray-500">Total Transaksi</p>
        <p class="text-2xl font-bold text-orange-500 mt-2">Rp {{ number_format($totalTransaksi, 0, ',', '.') }}</p>
    </div>
</div>
@endsection